...
These assert the exact shell command handed to docker exec. Under config transport,
every run-specific value rides in the command's "$1" config argv slot, so the
template is constant and the tests assert that property directly. No Docker required.
"""
...
def test_basic_variant_a_command():
    command = build(RecomRunSpec(pop_col="TOTPOP", assignment_col="CD", variant="A"))
    assert command == RECOM_COMMAND


def test_shadowed_subcommand_attribute_cannot_alter_command():
    # Regression: the sh -c token used to read run_spec.subcommand, which instance data
    # could shadow; the token must come from the type-keyed RUN_SPEC_SUBCOMMANDS mapping.
    run_spec = RecomRunSpec(pop_col="TOTPOP", assignment_col="CD", variant="A")
    setattr(run_spec, "subcommand", "chain; rm -rf /")

    assert build(run_spec) == RECOM_COMMAND
    assert command_config(run_spec)["command"] == "chain"


def test_run_spec_subclasses_have_no_subcommand():
    # Runner dispatch is exact-type, so unknown run-spec subclasses fail before command assembly.
    class CustomRunSpec(RecomRunSpec):
        pass

    run_spec = CustomRunSpec(pop_col="TOTPOP", assignment_col="CD", variant="A")
    with pytest.raises(TypeError, match="RecomRunnerConfig requires.*RecomRunSpec"):
        RecomRunnerConfig("./graphs/testing.json").run_command(run_spec)


def test_values_change_only_the_config_slot():
    first = RecomRunSpec(pop_col="TOTPOP", assignment_col="CD", variant="A")
    second = RecomRunSpec(
        pop_col="POP; false",
...
def test_bw_variant_is_reachable():
    ...
    config = command_config(run_spec)
...
def test_bendl_writer_config_names_output_file():
    ...
    assert build(run_spec) == RECOM_COMMAND
    config = command_config(run_spec)
    config_hash = RecomRunnerConfig("./graphs/testing.json").config_hash(run_spec)
...
def test_force_print_config_has_null_output():
    ...
    assert build(run_spec) == RECOM_COMMAND
    assert command_config(run_spec)["output_file"] is None
...
def test_key_bendl_graph_order_accepted():
    ...
    assert command_config(run_spec)["bendl_graph_order"] == "key:COUNTY"
...
def test_canonical_stdout_command_clears_bendl_graph_order():
    ...
    config = json.loads(runner.canonical_stdout_command(run_spec)[4])
...
def test_invalid_variant_rejected():
    ...
    with pytest.raises(ValueError):
        build(RecomRunSpec(pop_col="TOTPOP", assignment_col="CD", variant=cast(ChainVariant, "VW")))
...
def test_region_aware_variants_require_region_weights():
    with pytest.raises(ValueError, match="region_weights"):
        RecomRunSpec(pop_col="TOTPOP", assignment_col="CD", variant="AW")
...
def test_nan_region_weight_assigned_after_construction_never_reaches_the_container():
    ...
    run_spec.region_weights["COUNTY"] = float("nan")
    runner = RecomRunnerConfig("./graphs/testing.json")
    with pytest.raises(ValueError):
        runner.run_command(run_spec)
...
def test_post_construction_variant_and_writer_assignments_are_revalidated():
    ...
    run_spec = RecomRunSpec(pop_col="TOTPOP", assignment_col="CD", variant="A")
    run_spec.variant = "district-pairs-mst"  # long spelling normalizes at use
    assert command_config(run_spec)["variant"] == "district-pairs-mst"

    run_spec.variant = cast(ChainVariant, "Z")
    with pytest.raises(ValueError, match="Unknown variant 'Z'"):
        command_config(run_spec)

    run_spec.variant = "A"
    run_spec.writer = cast(Writer, "parquet")
...
def test_reversible_requires_balance_ub():
    with pytest.raises(ValueError, match="balance_ub"):
        RecomRunSpec(pop_col="TOTPOP", assignment_col="CD", variant="R")
...
def test_balance_ub_accepts_u32_bounds_for_non_reversible_variants(balance_ub: int):
    ...
    assert command_config(run_spec)["balance_ub"] == balance_ub
...
def test_unknown_writer_rejected():
    ...
    with pytest.raises(ValueError, match="writer"):
        RecomRunSpec(pop_col="TOTPOP", assignment_col="CD", variant="A", writer=cast(Writer, "csv"))
...
def test_recom_numeric_domains_rejected_at_construction(field, value):
    with pytest.raises(ValueError, match=field):
        RecomRunSpec(pop_col="TOTPOP", assignment_col="CD", variant="A", **{field: value})
...
def test_recom_config_field_types_rejected(field, value):
    ...
    with pytest.raises(ValueError, match=field):
        RecomRunSpec(**settings)


def test_recom_accepts_u64_seed_bound():
    run_spec = RecomRunSpec(pop_col="TOTPOP", assignment_col="CD", variant="A", rng_seed=2**64 - 1)
    assert command_config(run_spec)["rng_seed"] == 2**64 - 1
...
def test_assignment_column_must_be_safe_for_derived_file_names(assignment_col):
    with pytest.raises(ValueError, match="assignment_col must be a bare file name"):
        RecomRunSpec(pop_col="TOTPOP", assignment_col=assignment_col, variant="A")


def test_output_file_matches_container_output():
    config = RecomRunnerConfig("./graphs/testing.json", output_folder="./output")
    run_spec = RecomRunSpec(pop_col="TOTPOP", assignment_col="CD", variant="A")
    output_file = config.output_file(run_spec)
    assert output_file is not None
    config_hash = config.config_hash(run_spec)
    assert output_file.endswith(f"/output/testing/RecomA_CD_42_10_{config_hash}.jsonl")

    bendl_spec = RecomRunSpec(pop_col="TOTPOP", assignment_col="CD", variant="A", writer="bendl")
    bendl_file = config.output_file(bendl_spec)
    assert bendl_file is not None and bendl_file.endswith(".bendl")

    printed = RecomRunSpec(pop_col="TOTPOP", assignment_col="CD", variant="A", force_print=True)
